...
**Admission ternary:** `POST .../admission/explain` returns `decision` (`ACCEPT` \| `REJECT` \| `DEFER`), `resource_state`, and `demand`. Policy/contract/catalog quota → **REJECT** (existing 4xx on create). Insufficient but feasible capacity → **DEFER** (`202` + `admission_deferred` queue; scheduler flush). Fits now → **ACCEPT** (enqueue). Stats: `GET .../admission/stats` (`deferred_ratio`). Cluster knobs: `ML_AIR_CLUSTER_CPU`, `ML_AIR_CLUSTER_MEMORY_MB`, `ML_AIR_CLUSTER_GPU`, `ML_AIR_CLUSTER_MAX_TASKS`. Request demand: `override_config.resources` (`cpu`, `memory_mb`, `gpu`). Latency of explain/create: [evaluation harness](../guides/evaluation-harness.md).
