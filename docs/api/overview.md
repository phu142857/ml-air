...
**`GET /v1/runtime-config` → `features`:** the response includes boolean flags (for example **`strict_dataset_version_required`** and **`strict_dataset_version_all_post_runs`**) so UIs and integrators can mirror server-side pinning for generic run paths (`POST .../runs`, gated **`POST .../pipelines/{id}/run`**, **`POST .../check-readiness`**). See [readiness and gating](./readiness-and-gating.md) and [dataset version immutability](./dataset-version-immutability.md).
