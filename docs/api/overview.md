...
OpenAPI draft: [`openapi-v1-draft.yaml`](../../openapi-v1-draft.yaml). **Domain accountability:** `GET /v1/audit/events` (+ by id), project timeline `GET .../audit/timeline`, optional **`GET|POST .../domain-events/outbox`** and **`GET|POST|DELETE .../domain-webhooks/subscriptions`** when migrations **0050–0052** are applied — [Domain Events](../architecture/domain-events.md), [Audit Flow](../architecture/audit-flow.md). **Semantic realtime:** `GET|POST .../semantic-events/outbox` — [Realtime event envelope](./realtime-event-envelope.md).
