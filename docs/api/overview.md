...
- **search** (`GET .../search`), **identity** (`POST /v1/auth/login`, `POST /v1/auth/mfa/verify`, MFA enroll/disable, `GET|POST|DELETE /v1/auth/pats`, `GET|DELETE /v1/auth/sessions`, `GET /v1/auth/me`, users, service accounts — see [Login and Identity](../guides/login-and-identity.md)), **`GET /v1/auth/whoami`**, **plugins** (`/v1/plugins/...`)
- **traces** (`GET .../traces`, `GET .../traces/search`, `GET .../traces/{trace_id}`, `GET .../traces/{trace_id}/export`, `POST .../traces/ingest`) — [Traces API](./traces.md), [Trace explorer guide](../guides/use-trace-explorer.md)
...
**`GET /v1/runtime-config` → `features`:** includes **`domain_event_outbox`**, **`domain_webhook_delivery`**, **`semantic_event_outbox`**, **`semantic_webhook_delivery`**, **`opentelemetry`**, and dataset pinning flags — see [readiness and gating](./readiness-and-gating.md) and [Domain Events](../architecture/domain-events.md).

**`GET /v1/runtime-config` → `observability`:** optional **`grafana_ui_url`** (from **`ML_AIR_GRAFANA_URL`**, set when Grafana infra is enabled) for Hub links to lifecycle Grafana dashboards; see [View metrics](../guides/view-metrics.md). Trace APIs are documented in [Traces API](./traces.md). **`semantic_observability_surfaces`** is always present — machine-readable index of lifecycle surfaces → Prometheus metric names, label keys, related event types, and Grafana dashboard filenames (source: [`api/app/domains/observability/semantic_observability_model.py`](../../api/app/domains/observability/semantic_observability_model.py)).

OpenAPI draft: [`openapi-v1-draft.yaml`](../../openapi-v1-draft.yaml). **Domain accountability:** `GET /v1/audit/events` (+ by id), project timeline `GET .../audit/timeline`, optional **`GET|POST .../domain-events/outbox`** and **`GET|POST|DELETE .../domain-webhooks/subscriptions`** when migrations **0050–0052** are applied — [Domain Events](../architecture/domain-events.md), [Event Processing](../architecture/event-processing.md). **Semantic realtime:** `GET|POST .../semantic-events/outbox` — [Realtime event envelope](./realtime-event-envelope.md).

**Semantic lifecycle metrics (Phase 4):** API `/metrics` exports counters such as `mlair_lifecycle_*`, `mlair_readiness_blocked_total`, `mlair_eligibility_denied_total`, and dataset materialization series — see [View metrics](../guides/view-metrics.md). When **`infra.prometheus`** is enabled in all-in-one, the local Prometheus container loads rule file [`deploy/monitoring/alerts/mlair-alerts.yml`](../../deploy/monitoring/alerts/mlair-alerts.yml) (groups **`mlair-runtime`** and **`mlair-lifecycle-semantic`**). Grafana dashboards (e.g. **MLAir lifecycle (semantic metrics)**) are under [`deploy/monitoring/grafana/dashboards/`](../../deploy/monitoring/grafana/dashboards/) when **`infra.grafana`** is enabled.
...
- **Delete (maintainer)**: **`DELETE .../datasets/{dataset_id}`** removes the dataset, all versions, accumulation buffer, and related scope rows; **`DELETE .../datasets/{dataset_id}/versions/{version_id}`** removes one immutable snapshot (lineage edges for that version first). **`DELETE .../datasets/by-name/{dataset_name}`** resolves the dataset by name then deletes. Local `file://` artifacts are removed best-effort when `ML_AIR_DATASET_ARTIFACT_ROOT` points at writable storage. For policy-driven pruning of old versions, use retention **`POST .../datasets/{dataset_id}/retention/apply`** (see [dataset retention](./dataset-retention.md)).
