...
**`GET /v1/runtime-config` → `observability`:** optional **`jaeger_ui_url`** (from **`ML_AIR_JAEGER_UI_URL`**) for browser trace links when OpenTelemetry is enabled; see [OpenTelemetry guide](../guides/opentelemetry.md).
