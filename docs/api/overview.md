...
**`GET /v1/runtime-config` → `observability`:** optional **`jaeger_ui_url`** (from **`ML_AIR_JAEGER_UI_URL`**) for browser trace links when OpenTelemetry is enabled; see [OpenTelemetry guide](../guides/opentelemetry.md). **`semantic_observability_surfaces`** is always present — machine-readable index of lifecycle surfaces → Prometheus metric names, label keys, related event types, and Grafana dashboard filenames (source: [`api/app/domains/observability/semantic_observability_model.py`](../../api/app/domains/observability/semantic_observability_model.py)); see [View metrics](../guides/view-metrics.md).
