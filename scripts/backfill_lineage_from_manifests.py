...
def _db_url() -> str:
    return os.getenv("ML_AIR_DATABASE_URL", "postgresql://mlair:mlair@localhost:35432/mlair")
...
def _load_rows(
    *,
    ...
    from psycopg import connect  # local import so --help works without db deps installed
    ...
    try:
        ...
        if exc.name == "psycopg":
            print(
                ...
                "Use `make backfill-lineage` (runs inside api container), "
                "or install local deps for api script execution."
            )
            return 2
        raise
