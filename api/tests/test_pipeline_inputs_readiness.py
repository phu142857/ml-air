...
class PipelineInputsReadinessTests(unittest.TestCase):
    @patch("app.domains.lifecycle.readiness_service._dataset_actual_size", return_value=("ds-cv", 30))
    def test_blocks_when_actual_below_pipeline_required_size(self, _mock_size) -> None:
        out = rs.evaluate_pipeline_inputs_readiness(
            tenant_id="t",
            project_id="p",
            pipeline_config={
                "inputs": [{"dataset": "cv-traffic-frames", "required_size": 50}],
                "tasks": [],
            },
    ...
    @patch(
        ...
        return_value={"dataset_id": "ds-cv", "record_count": 60},
    )
    @patch("app.domains.lifecycle.readiness_service._dataset_actual_size", return_value=("ds-cv", 12))
    def test_uses_pinned_version_record_count_for_matching_input_dataset(
        ...
        out = rs.evaluate_pipeline_inputs_readiness(
            tenant_id="t",
            project_id="p",
            pipeline_config={"inputs": [{"dataset": "cv-traffic-frames", "required_size": 50}]},
    ...
    @patch(
        ...
        side_effect=lambda _t, _p, name: ("ds-cv", 12) if name == "cv-traffic-frames" else ("ds-upload", 30),
    )
    @patch(
        "app.domains.lifecycle.lineage_service.get_dataset_version",
    ...
    def test_pin_does_not_apply_to_different_pipeline_input_dataset(self, _mock_gdv, _mock_das) -> None:
        """Pin on upload must not satisfy cv-traffic-frames input row."""
        out = rs.evaluate_pipeline_inputs_readiness(
            tenant_id="t",
            project_id="p",
            pipeline_config={"inputs": [{"dataset": "cv-traffic-frames", "required_size": 50}]},
        ...
        self.assertFalse(out["ready"])
        self.assertEqual(out["blocking_datasets"][0]["dataset"], "cv-traffic-frames")
